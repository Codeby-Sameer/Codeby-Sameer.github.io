...
const IndustrySolutions = () => {
  const solutions = {
    chAInIQ: {
      ...
      description:
        "ChAInIQ is GC Technologies' advanced distributed quality management solution designed to address the supply chain challenges faced by pharmaceutical companies, particularly on the manufacturing floor. The solution is tailored for injectable production lines, where efficiency, precision, and compliance are critical.",
      fullDescription:
        "ChAInIQ transforms how pharmaceutical companies manage their production lines by integrating IoT and AI technology, optimizing operations, and providing deep insights into every aspect of the supply chain. This solution not only addresses immediate manufacturing challenges but also ensures long-term operational success and product quality in the highly regulated pharma sector.",
      icon: FaPills,
      ...
      features: [
        {
          ...
          description:
            "Atlas uses embedded AI to intelligently match candidates to job roles, ensuring faster and more accurate hiring decisions tailored to small and medium businesses.",
          icon: FaBrain,
        },
        {
          title: "Seamless API Integrations",
          description:
            "Integrated with Digiverifier and Hirex.ai, Atlas automates candidate verification and advanced resume parsing, streamlining the recruitment process with reliable and verified data.",
          icon: FaLink,
        },
        {
          title: "Efficiency and Scalability",
          description:
            "Designed for scalability, Atlas simplifies every stage of the recruitment process—from job posting to onboarding—allowing businesses to grow while maintaining recruitment efficiency.",
          icon: FaBolt,
        },
      ],
      benefits: [
...
const SolutionCard = ({ solution }) => {
  ...
  return (
    <div className="animate-fade-in-up">
      <div
        className={`bg-gradient-to-r ${solution.gradient} text-white overflow-hidden border border-gray-100 rounded-2xl`}
      >
        {/* Solution Header */}
        <div className="text-white p-8">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between">
            <div className="flex items-center mb-4 md:mb-0">
              <div className="w-14 h-14 rounded-2xl bg-white flex items-center justify-center mr-4 shadow-md">
                <Icon className={`text-3xl ${solution.text}`}  />
              </div>
              <div>
                <h2 className="text-3xl font-bold">{solution.name}</h2>
                <p className="text-xl opacity-90 mt-1">{solution.title}</p>
                {solution.tagline && (
                  <p className="text-lg opacity-80 mt-2 italic">
                    {solution.tagline}
                  </p>
                )}
              </div>
...
const FeatureCard = ({ feature, index, solutionName }) => {
  ...
  return (
    ...
    >
      <div className="mb-4">
        <div className="inline-flex items-center justify-center w-12 h-12 rounded-full bg-primary/10 text-primary group-hover:bg-primary group-hover:text-white transition-all duration-300">
          <Icon className="text-xl" />
        </div>
      </div>
      <h4 className="text-lg font-bold text-darkBlue mb-3">
        {feature.title}
      </h4>
